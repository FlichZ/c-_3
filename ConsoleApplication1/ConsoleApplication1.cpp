...
public:
    Car(string make, string model, int year, int speed, int num_doors, bool has_sunroof)
        : Vehicle(make, model, year, speed), num_doors_(num_doors), has_sunroof_(has_sunroof) {}

    int GetNumDoors() const { return num_doors_; }
    bool HasSunroof() const { return has_sunroof_; }

    void PrintDetails() const override {
        Vehicle::PrintDetails();
        cout << "Количество дверей: " << num_doors_ << endl;
        cout << "Имеет люк на крыше: " << (has_sunroof_ ? "да" : "нет") << endl;
    }
...
public:
    Truck(string make, string model, int year, int speed, int max_load, bool is_4x4)
        : Vehicle(make, model, year, speed), max_load_(max_load), is_4x4_(is_4x4) {}

    int GetMaxLoad() const { return max_load_; }
    bool Is4x4() const { return is_4x4_; }

    void PrintDetails() const override {
        Vehicle::PrintDetails();
        cout << "Максимальная нагрузка: " << max_load_ << endl;
        cout << "Это 4x4: " << (is_4x4_ ? "да" : "нет") << endl;
    }
...
int main() {
    Car* car1 = new Car("Honda", "Civic", 2010, 100, 4, false);
    Car* car2 = new Car("Toyota", "Camry", 2005, 90, 4, true);
    Truck* truck1 = new Truck("Ford", "F150", 2015, 80, 2, 5000);
    Truck* truck2 = new Truck("Chevy", "Silverado", 2020, 70, 4, 6000);
